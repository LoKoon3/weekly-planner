...
const WeeklyCalendar: React.FC<WeeklyCalendarProps> = ({ weekDays, onTaskClick, onEmptySlotClick, onDateClick }) => {
  ...
  return (
    <div className="flex-1 bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-2xl shadow-sm flex flex-col overflow-hidden">
      {/* Header - 날짜 클릭 가능 */}
      <div className="flex border-b dark:border-gray-700 ml-20">
        {weekDays.map((day, idx) => {
          ...
          return (
            <div
              ...
              className="flex-1 py-3 text-center border-l dark:border-gray-700 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors"
            >
              <div className={`text-xs font-bold uppercase tracking-widest ${isToday ? 'text-blue-600' : 'text-gray-400'}`}>
                {WEEKDAYS[idx]}
              </div>
              <div className={`text-xl font-black mt-0.5 ${isToday ? 'text-blue-600' : 'dark:text-white'}`}>
                {day.getDate()}
              </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {TIME_PERIODS.map((period) => (
          <div key={period.id} className="flex border-b dark:border-gray-700 last:border-b-0">
            {/* Time Period Label */}
            <div className={`w-20 shrink-0 p-3 border-r dark:border-gray-700 ${period.color}`}>
            ...
            {dayColumns.map((col, colIdx) => (
              <div
                key={colIdx}
                className={`flex-1 min-h-[80px] p-2 border-l dark:border-gray-700 ${period.color}`}
                onClick={() => {
                ...
                <div className="space-y-1.5">
                  {col.tasksByPeriod[period.id]?.map(task => (
                    <div
                      ...
                      className={`p-2 rounded-lg border-l-4 shadow-sm cursor-pointer transition-all hover:scale-[1.02] hover:shadow-md group
                        ${PRIORITY_COLORS[task.priority]}
                        ${task.completed ? 'opacity-50 grayscale' : 'opacity-100'}
                      `}
                    >
                      <div className="flex items-start justify-between gap-1">
                        <h4 className={`text-xs font-bold leading-tight line-clamp-2 ${task.completed ? 'line-through' : ''}`}>
                          {task.title}
                        </h4>
                        <button
                          onClick={(e) => {
                      ...
                      <div className="mt-1 flex flex-wrap gap-1">
                        <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-black/10">
                          {task.startTime}
                        </span>
                        {task.timeTag && (
                          <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${TIME_TAG_COLORS[task.timeTag]}`}>
                            #{task.timeTag}
                          </span>
                        )}
                      </div>
